...
void RendererMediaPlayerManager::EndVideo(int playID) {
	/*
	double duration = 0;
	WebMediaPlayerAndroid* player = GetMediaPlayer(playID);
    if (player)
      duration = player->duration();
	Seek(playID, base::TimeDelta::FromSeconds(duration-1));
	Start(playID);*/
	OnMediaPlaybackCompleted(playID);
	LOG(INFO) << "EndVideo: " << playID;
}
