...
void ContentViewCoreImpl::EndVideo(int playID) {
	RenderViewHost* host = web_contents_->GetRenderViewHost();
    if (host)
      host->Send(new ViewMsg_EndVideo(host->GetRoutingID(), playID));
}
...
void ContentViewCoreImpl::notifyVideoUrl(const GURL& url, int playerID) {
  JNIEnv* env = AttachCurrentThread();
  ...
  if (obj.is_null())
    return;
  ScopedJavaLocalRef<jstring> j_url = ConvertUTF8ToJavaString(env, url.spec());
  Java_ContentViewCore_notifyVideoUrl(env, obj.obj(),
  	                                  j_url.obj(),
  	                                  playerID);
}
...
void ContentViewCoreImpl::PauseVideo(JNIEnv* env, jobject obj) {
  PauseVideo();
}

void ContentViewCoreImpl::NotifyEndVideo(JNIEnv* env, jobject obj, jint playID) {
	EndVideo(playID);
}
